function(input, output, session) {
    ...
    if(input$index == 'Metric Overview'){
      ...
      tagList(
        tabsetPanel(
          tabPanel("Charts", 
                   fluidRow(
                     column(6, plotOutput('plot1_overview')),
                     column(6, plotOutput('plot2_overview'))),
                   fluidRow(
  ...
  output$plot1_overview <- renderPlot({
    ...
    ggplot(overview_data, aes(x = !!sym(input$overviewparameter))) +
  ...
  #BOXPLOT AND VIOLIN PLOT FOR METRIC OVERVIEW
  output$plot2_overview <- renderPlot({
    req(input$year, input$overviewparameter)
      boston_weather |>
        filter(Year %in% input$year) |>
        ggplot(aes(y = factor(Year), x = !!sym(input$overviewparameter))) +
        geom_violin(fill = "red", alpha = 0.3) +  # Add violin plot
